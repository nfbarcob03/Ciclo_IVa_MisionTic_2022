...
class ControladorInscripcion():
    def __init__(self):
        ...
        self.repositorioDepartamentos = RepositorioDepartamento()
    def index(self):
    ...
    "Obtener suma de notas en materia"

    def sumaNotasEnMateria(self, id_materia):
        return self.repositorioInscripcion.sumaNotasEnMateria(id_materia)

    def reporteTotalNotasPorMateriaCompleto(self, id_estudiante = None):
        return self.repositorioInscripcion.reporteTotalNotasPorMateriaCompleto(id_estudiante)

    def reporteTotalNotasPorMateriaQueryRaiz(self,id_estudiante = None):
        result = []
        querySet = self.repositorioInscripcion.reporteTotalNotasPorMateriaQueryRaiz(id_estudiante)
        for registro in querySet:
            materia = self.repositorioMaterias.findById(registro['_id'].split('(')[2].replace("'","").replace(")",""))
            departamento = self.repositorioDepartamentos.findById(materia['departamento']['_id'])
            result.append({"total":registro['sumaNotasMateria'],"departamento":departamento})
        return result